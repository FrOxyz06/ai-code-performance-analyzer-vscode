...
test('Node bridge runs the actual Python benchmark with example files', async () => {
    const request = { ...JSON.parse(fs.readFileSync('examples/benchmark.json')), before: fs.readFileSync('examples/example.py', 'utf8'), after: fs.readFileSync('examples/candidate.py', 'utf8') };
